                        ...
                        <div class="student-widget select-plan-group">
                            <div class="student-widget-group">
                                ...
                                <div class="payment-btn">
                                    ...
                                    <a href="javascript:void(0)" id="paypal-course-button" class="d-none">Paypal Button</a>
                                    <div class="d-none" id="paypal-course-button1"></div>
...
"></script>
<script src="https://www.paypal.com/sdk/js?client-id={{env('PAYPAL_CLIENT_ID')}}&vault=true&commit=true&locale=en_US&components=messages,buttons&disable-funding=card"></script>
...
<script type="text/javascript">
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if(response.success==true) {
                        ...
                        // $('#paypal-course-button1').removeClass('d-none');
        ...
        $(document).on('click', '#paypal-course-button', function () {
            var formData = new FormData($('#createFrm')[0]);
            formData.append('_token', '{{ csrf_token() }}'); // Add this line to include the CSRF token

            $.ajax({
                type: "post",
                url: "{{ url('student/paypal-course-order-create') }}",
                data: formData, 
                datatype:'json',
                contentType: false,
                processData: false,
                success: function (resJson) {
                    ...
                    console.log(resJson);
                    $('#pl').val(resJson.data.plan_id);
                    $('#paypal-course-button').addClass('d-none');
                    $('#paypal-course-button1').removeClass('d-none');
                },
...
//             var formData = new FormData($('#createFrm')[0]);
//                 return fetch(
//                         "{{url('student/paypal-course-order-create')}}",
//                         {
//                         method: 'POST',
//                         body: formData
//                         }
    ...
    paypal.Buttons({
        ...
        onApprove: function(data, actions) {
            // Subscription has been approved
                ...
                $.ajax({
                        type:"post",
                        url:"{{url('student/paypal-order-capture')}}",
                        data:{'plan_id':pla,'orderid':orderid,'subscriptionid':subscriptionid,"_token": "{{ csrf_token() }}"},
                        success:function(data)
                        {
