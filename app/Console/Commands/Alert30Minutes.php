...
class Alert30Minutes extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'Alert:30Minutes';
     ...
     * @var string
     */
    protected $description = 'Alert for student and teacher both just before 30 minutes for session time';
    ...
    public function handle()
    {
        ...
        $date_time = date('Y-m-d H:i', strtotime('+30 minutes'));

        $data = BookSession::where('start_time','=',$date_time)->where('is_cancelled',0)->with('Student.StudentDetail.TimeZone')->with('Teacher.TeacherSetting.TimeZone')->get(); //where('start_time','=',$date_time)
        ...
        foreach($data as $val)
        {
            ...
            if($val->Student!=null)
            {
                ...
                Mail::send('email.student-class-30-min-reminder', $email, function ($messages) use ($email) {
                    $messages->to($email['sender_email'])
                                ->from($email['inext_email'], 'Latogo');
                    $messages->subject("Latogo Class 30 Minutes Reminder");
                });
            }
            ...
            if($val->Teacher!=null)
            {
                ...
                Mail::send('email.teacher-class-30-min-reminder', $email, function ($messages) use ($email) {
                    $messages->to($email['sender_email'])
                                ->from($email['inext_email'], 'Latogo');
                    $messages->subject("Latogo Class 30 Minutes Reminder");
                });
        ...
        // \Log::info('30 Minutes Alert Running');
    }
}
