...
class CourseLessionController  extends Controller
{
    //
    public function index(Request $request)
    {
        if($request->ajax())
        {
            ...
            $user = DB::table('course_lessions')->where('status','<>',2)->get();
            ...
            ->addColumn('action', function($row) {
                ...
                $edit_url = url('/admin/course_lession/edit',['id'=>base64_encode($row->id)]);
    ...
    public function create(Request $request)
    {
        ...
        try{
            ...
            if($request->file('image')){
                ...
                $destination_path = public_path('/uploads/course_lessions/');
        ...
        if($request->file('lession')){

            $lession = $request->file('lession');
            $date = date('YmdHis');
            $no = str_shuffle('123456789023456789034567890456789905678906789078908909000987654321987654321876543217654321654321543214321321211');
            $random_no = substr($no, 0, 2);
            $final_lession = $date.$random_no.'.'.$lession->getClientOriginalExtension();
            ...
            $lession->move($destination_path , $final_lession);
            ...
            $data = [
                ...
                'lession' => !empty($final_lession) ? $final_lession:NULL,
                'status' => 1,
                'created_at' => date('Y-m-d H:i:s')
            ];

            DB::table('course_lessions')->insert($data);
    ...
    public function edit(Request $request)
    {
        ...
        if($request->isMethod('get'))
        {
            $lession = DB::table('course_lessions')->where('id',$customer_id)->first();

            return view('admin.lession.edit',compact('lession'));
        }
        ...
        $about = DB::table('course_lessions')->where('id',$customer_id)->first();
        ...
        try{
            ...
            DB::table('course_lessions')->where('id',$customer_id)->update($data);
