...
class BannerPointController extends Controller
{
    //
    public function index(Request $request)
    {
        if($request->ajax())
        {
            ...
            $user = DB::table('banner_points')->where('status','<>',2)->get();
            ...
            ->addColumn('action', function($row) {
                ...
                $edit_url = url('/admin/banner_point/edit',['id'=>base64_encode($row->id)]);
        ...
        return view('admin.banner-point.index');
    }
    ...
    public function create(Request $request)
    {
        ...
        {
            return view('admin.banner-point.create');
            // return view('admin.user.create');
        }

        $rules = [
            'point' => 'required|min:1|max:255',
            
            'point_image' => 'nullable|mimes:jpeg,jpg,bmp,png,gif,svg',
        ...
        try{
            ...
            if($request->file('point_image')){

                $image = $request->file('point_image');
                $date = date('YmdHis');
                $no = str_shuffle('123456789023456789034567890456789905678906789078908909000987654321987654321876543217654321654321543214321321211');
                $random_no = substr($no, 0, 2);
                $final_image_name = $date.$random_no.'.'.$image->getClientOriginalExtension();

                $destination_path = public_path('/uploads/banner_point/');
            ...
            $data = [
                ...
                'point_image' => !empty($final_image_name) ? $final_image_name:NULL,
                'status' => 1,
                'created_at' => date('Y-m-d H:i:s')

            ];

            DB::table('banner_points')->insert($data);
    ...
    public function edit(Request $request)
    {
        ...
        if($request->isMethod('get'))
        {
            $banner1 = DB::table('banner_points')->where('id',$customer_id)->first();

            return view('admin.banner-point.edit',compact('banner1'));
        }
        ...
        $about = DB::table('banner_points')->where('id',$customer_id)->first();
        ...
        try{
            ...
            {
                $final_image_name = $about->point_image;
            }
            ...
            DB::table('banner_points')->where('id',$customer_id)->update($data);
